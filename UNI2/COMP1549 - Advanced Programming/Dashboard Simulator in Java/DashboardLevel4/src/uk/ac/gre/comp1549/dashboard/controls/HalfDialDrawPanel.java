...
import javax.swing.*;
/**
 * HalfDialDrawPanel. Draw a half dial and indicate current value.
 *
 * @author Gyokay Ali
 * @version 3.0
 */
public class HalfDialDrawPanel extends JPanel implements InterfacePanels,InterfaceDrawHands {
    ...
    private JLabel lblTitle; // indicator's title
    /**
    ...
    public HalfDialDrawPanel(int radius, int padding, int dialMaxValue, int value) {
        ...
        this.value = value;
        handLength = 0.9 * radius; // hand length is fixed at 80% of radius
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        g2.drawString("10", 10, 64);
        g2.drawString("30", 8, 45);
        g2.drawString("50", 18, 27);
        g2.drawString("80", 41, 18);
        
        // draw centre of the dial as a small circle of fixed size
        Ellipse2D circle = new Ellipse2D.Double((radius + padding) - 4, (radius + padding) - 4, 8, 8);
        g2.fill(circle);
    ...
    public void setValue(int value) {
        // don't let the value go over the maximum for the dial or under the minimum
